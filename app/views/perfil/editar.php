        ...
        <form action="<?= URL_BASE ?>/perfil/atualizar" method="POST" id="form-editar-perfil" enctype="multipart/form-data" class="space-y-3">
            ...
            <div class="flex flex-col items-center mb-6">
                ...
                <div class="relative <?= $tipoPerfil !== 'administrador' ? 'cursor-pointer group' : '' ?>" <?= $tipoPerfil !== 'administrador' ? 'onclick="abrirSeletorFoto()"' : '' ?>>
                    <div class="w-32 h-32 rounded-full border-4 border-roxinhoFofo overflow-hidden bg-surface flex items-center justify-center shadow p-1">
                        <?php
                    ...
                    <?php if ($tipoPerfil !== 'administrador'): ?>
                        <div class="absolute bottom-1 right-1 bg-surface p-2 rounded-full shadow border border-rosa-2 text-text-muted group-hover:bg-rosa-1 transition">
                            ✏️
                        </div>
                    <?php endif; ?>
                </div>
            ...
            <div class="bg-surface rounded-2xl shadow-sm overflow-hidden border border-rosa-2">
                <button type="button" class="w-full px-5 py-4 flex justify-between items-center bg-rosa-1/20 hover:bg-rosa-1/30 transition focus:outline-none" onclick="toggleAccordion('acc-sobre')">
                ...
                <div id="acc-sobre" class="hidden px-5 py-4 space-y-4 border-t border-rosa-2">
                    ...
                    <div>
                        ...
                        <input type="date" value="<?= htmlspecialchars($usuario['dt_nasc'] ?? '') ?>" disabled class="input-padrao bg-surface/50 text-text-muted cursor-not-allowed">
                    ...
                    <?php if (in_array($tipoPerfil, ['ong', 'protetor'])): ?>
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <input type="text" name="codigo_documento" id="codigo_documento" 
                                       ...
                                       readonly 
                                       class="input-padrao bg-surface/50 text-text-muted cursor-not-allowed transition-colors">
                        ...
                        <div id="container-novo-comprovante" class="hidden p-3 bg-aviso/10 border border-aviso/30 rounded-xl space-y-2">
                            ...
                            <div>
                                <label class="block text-xs font-bold text-text-dark mb-1">Novo Comprovante de Atividade (PDF ou Imagem) *</label>
                                <input type="hidden" name="comprovante_atual" value="<?= htmlspecialchars($especifico['comprovante_documento'] ?? '') ?>">
                                <input type="file" name="comprovante_documento" id="comprovante_documento" accept=".pdf, .jpg, .jpeg, .png" class="input-padrao bg-surface text-xs py-2">
                            </div>
                        </div>
            ...
            <?php if ($tipoPerfil !== 'administrador'): ?>
            <div class="bg-surface rounded-2xl shadow-sm overflow-hidden border border-rosa-2">
                <button type="button" class="w-full px-5 py-4 flex justify-between items-center bg-rosa-1/20 hover:bg-rosa-1/30 transition focus:outline-none" onclick="toggleAccordion('acc-local')">
                ...
                <div id="acc-local" class="hidden px-5 py-4 space-y-4 border-t border-rosa-2">
                    <div class="relative">
            ...
            <?php if ($tipoPerfil === 'adotante' || $tipoPerfil === 'usuario'): ?>
                ...
                ?>
            <div class="bg-surface rounded-2xl shadow-sm overflow-hidden border border-rosa-2">
                <button type="button" class="w-full px-5 py-4 flex justify-between items-center bg-rosa-1/20 hover:bg-rosa-1/30 transition focus:outline-none" onclick="toggleAccordion('acc-pref')">
                ...
                <div id="acc-pref" class="hidden px-5 py-4 space-y-4 border-t border-rosa-2">
                    
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <label class="label-padrao">Moradia</label>
                            <select name="tipo_morada" class="input-padrao bg-surface text-sm">
                        ...
                        <div>
                            ...
                            <select name="tamanho_interno_morada" class="input-padrao bg-surface text-sm">
                    ...
                    <div>
                        ...
                        <select name="espaco_externo" class="input-padrao bg-surface text-sm">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <select name="possui_criancas" class="input-padrao bg-surface text-sm">
                        ...
                        <div>
                            ...
                            <select name="possui_outros_pets" class="input-padrao bg-surface text-sm">
                    ...
                    ?>
                    <div>
                        <span class="text-sm font-medium text-text-dark block mb-2">Espécie:</span>
                        <div class="space-y-2">
                            <label class="flex items-center gap-2 p-2 bg-surface border border-rosa-2 rounded-lg cursor-pointer hover:bg-rosa-1/20 text-sm">
                        ...
                        <div id="container-outras-especies" class="hidden mt-3 p-3 border border-rosa-2 rounded-lg bg-surface">
                            <label class="block font-medium mb-2 text-xs text-text-dark">Selecione outras espécies desejadas:</label>
                    ...
                    <div class="grid grid-cols-2 gap-4 mt-2">
                        <div>
                            <span class="text-sm font-medium text-text-dark block mb-2">Porte:</span>
                            <div class="space-y-2">
                                <label class="flex items-center gap-2 p-2 bg-surface border border-rosa-2 rounded-lg cursor-pointer hover:bg-rosa-1/20 text-xs"><input type="checkbox" name="preferencias_porte[]" value="pequeno" <?= in_array('pequeno', $prefPorte) ? 'checked' : '' ?> class="text-roxinhoFofo focus:ring-roxinhoFofo rounded"> Pequeno</label>
                                <label class="flex items-center gap-2 p-2 bg-surface border border-rosa-2 rounded-lg cursor-pointer hover:bg-rosa-1/20 text-xs"><input type="checkbox" name="preferencias_porte[]" value="medio" <?= in_array('medio', $prefPorte) ? 'checked' : '' ?> class="text-roxinhoFofo focus:ring-roxinhoFofo rounded"> Médio</label>
                                <label class="flex items-center gap-2 p-2 bg-surface border border-rosa-2 rounded-lg cursor-pointer hover:bg-rosa-1/20 text-xs"><input type="checkbox" name="preferencias_porte[]" value="grande" <?= in_array('grande', $prefPorte) ? 'checked' : '' ?> class="text-roxinhoFofo focus:ring-roxinhoFofo rounded"> Grande</label>
                            </div>
                        </div>
                        <div>
                            <span class="text-sm font-medium text-text-dark block mb-2">Sexo:</span>
                            <div class="space-y-2">
                                <label class="flex items-center gap-2 p-2 bg-surface border border-rosa-2 rounded-lg cursor-pointer hover:bg-rosa-1/20 text-xs"><input type="checkbox" name="preferencias_sexo[]" value="femea" <?= in_array('femea', $prefSexo) ? 'checked' : '' ?> class="text-roxinhoFofo focus:ring-roxinhoFofo rounded"> Fêmea</label>
                                <label class="flex items-center gap-2 p-2 bg-surface border border-rosa-2 rounded-lg cursor-pointer hover:bg-rosa-1/20 text-xs"><input type="checkbox" name="preferencias_sexo[]" value="macho" <?= in_array('macho', $prefSexo) ? 'checked' : '' ?> class="text-roxinhoFofo focus:ring-roxinhoFofo rounded"> Macho</label>
                            </div>
                        </div>
                    </div>
            ...
            <div class="bg-surface rounded-2xl shadow-sm overflow-hidden border border-rosa-2">
                <button type="button" class="w-full px-5 py-4 flex justify-between items-center bg-rosa-1/20 hover:bg-rosa-1/30 transition focus:outline-none" onclick="toggleAccordion('acc-seguranca')">
                ...
                <div id="acc-seguranca" class="hidden px-5 py-4 space-y-4 border-t border-rosa-2">
                    <div>
                        <label class="label-padrao">E-mail Atual</label>
                        <p class="text-sm font-bold text-text-dark mb-2"><?= htmlspecialchars($emailMascarado ?? '') ?></p>
                        <a href="<?= URL_BASE ?>/perfil/trocar-email" class="inline-block bg-roxinhoFofo text-white py-2 px-4 rounded-xl font-bold text-xs hover:opacity-90 transition shadow-sm">
                            ✉️ Trocar E-mail
                        </a>
                    </div>
                    <hr class="border-rosa-2 my-2">
