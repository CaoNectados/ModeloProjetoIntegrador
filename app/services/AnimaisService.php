...
class AnimaisService
{
    ...
    public function cadastrarAnimal(Animais $animal): void
    {
        ...
        $resultado = $this->animaisRepository->cadastrarAnimal($animal);
    ...
    public function editarAnimal(Animais $animal): void
    {
        $this->validarAnimal($animal);
        $this->validarPermissaoDoProtetor($animal);

        $atualizado = $this->animaisRepository->editarAnimal($animal);
    ...
    public function atualizarStatus(Animais $animal): void
    {
        ...
        $atualizado = $this->animaisRepository->alterarStatus($animal->getAnimalId(), $animal->getStatus());
    ...
    public function desativarAnimal(Animais $animal): void
    {
        ...
        $excluido = $this->animaisRepository->excluirLogico($animal->getAnimalId());
    ...
    public function reativarAnimal(Animais $animal): void
    {
        ...
        $reativado = $this->animaisRepository->reativarAnimal($animal->getAnimalId());
    ...
    private function validarAnimal(Animais $animal): void
    {
    ...
    {
        $statusPermitidos = ['disponivel', 'adotado', 'indisponivel'];

        if (!in_array($status, $statusPermitidos, true)) {
            throw new InvalidArgumentException('Status inválido. Os valores permitidos são: disponível, adotado e indisponível.');
        }
    }
    ...
    private function validarPermissaoDoProtetor(Animais $animal): void
    {
        ...
        $animalBanco = $this->animaisRepository->buscarPorId($animal->getAnimalId() ?? 0);
